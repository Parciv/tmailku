...
export async function resolveAddress(env: Env, toAddr: string) {
	...
	return env.DB.prepare(
		`SELECT a.*, d.status AS domain_status FROM addresses a
...
export async function storeEmail(env: Env, source: 'routing' | 'imap', p: ParsedEmail): Promise<string | null> {
	...
	if (!address || address.blocked || address.domain_status === 'disabled') {
		await addLog(env, 'warn', 'email', `dropped (no/disabled address) ${p.toAddr}`)
		return null
	}

	const emailId = uid('em')
	const otp = detectOtp(p.subject || '', p.text || stripHtml(p.html || ''))
	const maxBytes = (Number(settings['max_attachment_mb'] || '10') || 10) * 1024 * 1024
	...
	for (const att of atts) {
		...
		await env.DB.prepare(
			...
			.bind(attId, emailId, att.filename ?? 'file', att.mimeType ?? 'application/octet-stream', bytes.byteLength, r2Key)
			.run()
	}
	if (hasAttachment) {
		await env.DB.prepare('UPDATE emails SET has_attachment = 1 WHERE id = ?').bind(emailId).run()
	}

	await addLog(env, 'info', 'email', `email.received -> ${p.toAddr} (from: ${p.fromAddr || '?'})`, {
		emailId,
		source,
		otp: !!otp,
	})
	...
	await env.KV.put(`inbox_ver:${address.address}`, String(now()), { expirationTtl: 86400 })
