...
async function pickDomain(env: Env, requested?: string) {
	if (requested) {
		return env.DB.prepare("SELECT * FROM domains WHERE domain = ? AND status = 'active'").bind(requested).first<any>()
	}
	return env.DB.prepare("SELECT * FROM domains WHERE status = 'active' ORDER BY RANDOM() LIMIT 1").first<any>()
}
...
export async function createAddress(env: Env, ip: string, body: any) {
	...
	if (!(await rateLimit(env, 'addr:' + ip, limit))) return { status: 429, json: { error: 'rate limited' } }

	const domain = await pickDomain(env, body?.domain)
	if (!domain) return { status: 400, json: { error: 'tidak ada domain aktif' } }

	const local = (body?.local || randomLocalPart(settings['address_format'] || 'word+num'))
		.toLowerCase()
		.replace(/[^a-z0-9._-]/g, '')
	const address = local + '@' + domain.domain
	const ttlMin = Number(body?.ttlMinutes || settings['ttl_minutes'] || '60') || 60
	...
	return { status: 200, json: { address, ownerToken, expiresAt: now() + ttlMin * 60000, domain: domain.domain } }
}
...
publicRoutes.get('/domains', async (c) => {
	const { results } = await c.env.DB.prepare("SELECT domain FROM domains WHERE status = 'active' ORDER BY domain").all<{ domain: string }>()
...
publicRoutes.get('/address/:addr', async (c) => {
	...
	return c.json({ address: row.address, expiresAt: row.expires_at, createdAt: row.created_at })
})

// daftar email inbox (ringkas)
export async function listInbox(env: Env, addr: string) {
	const address = await env.DB.prepare('SELECT id FROM addresses WHERE address = ?').bind(addr).first<{ id: string }>()
...
publicRoutes.get('/email/:id', async (c) => {
	...
	if (!row) return c.json({ error: 'not found' }, 404)
	await c.env.DB.prepare('UPDATE emails SET seen = 1 WHERE id = ?').bind(row.id).run()
...
publicRoutes.get('/email/:id/raw', async (c) => {
	const row = await c.env.DB.prepare('SELECT raw_r2_key FROM emails WHERE id = ?').bind(c.req.param('id')).first<any>()
	if (!row?.raw_r2_key) return c.json({ error: 'not found' }, 404)
	const obj = await c.env.R2.get(row.raw_r2_key)
	if (!obj) return c.json({ error: 'not found' }, 404)
	return new Response(obj.body, { headers: { 'content-type': 'message/rfc822' } })
})
...
publicRoutes.delete('/email/:id', async (c) => {
	await c.env.DB.prepare('DELETE FROM attachments WHERE email_id = ?').bind(c.req.param('id')).run()
