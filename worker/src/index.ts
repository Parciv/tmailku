...
export default {
	...
	async email(message: ForwardableEmailMessage, env: Env, ctx: ExecutionContext) {
		try {
			...
			await addLog(env, 'error', 'email', 'gagal proses email routing: ' + (e?.message || e))
		}
	},
	...
	async scheduled(_event: ScheduledController, env: Env, ctx: ExecutionContext) {
		ctx.waitUntil(
			(async () => {
				await pollAllImap(env).catch((e) => addLog(env, 'error', 'cron', 'imap poll: ' + e))
				await cleanupExpired(env).catch((e) => addLog(env, 'error', 'cron', 'cleanup: ' + e))
			})(),
		)
	},
}
